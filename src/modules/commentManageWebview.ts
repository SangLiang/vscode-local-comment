...
export class CommentManageWebviewPanel {
                    ...
                    switch (message.command) {
                        ...
                        case IPC_MESSAGES.OPEN_COMMENT_ROW:
                            if (message.filePath != null && message.line != null) {
                                await this._openRow(message.filePath, message.line, message.id);
                            }
                            return;
    ...
    public refreshRows(params?: {
        ...
        const comments = this._getCommentsForViewing();
        let matchedLineMap: Map<string, number> | undefined;

        if (this._isActiveGroup()) {
            matchedLineMap = new Map();
            for (const filePath of Object.keys(comments)) {
                try {
                    const uri = vscode.Uri.file(filePath);
                    const matched = this._commentManager.getComments(uri);
                    for (const m of matched) {
                        matchedLineMap.set(m.id, m.line);
                    }
                } catch {
                    // 文件可能无法访问，跳过
                }
        ...
        let rows = flattenCommentsToRows(comments, workspaceRoot, matchedLineMap);
        const allRows = rows;
    ...
    private async _openRow(filePath: string, line: number, id?: string): Promise<void> {
        try {
            const absPath = this._resolveAbsPath(filePath);
            if (id) {
                await vscode.commands.executeCommand(COMMANDS.GO_TO_COMMENT, absPath, line, id);
                return;
            }
