...
- **预览与导出**：在 `.md` 中按 **Ctrl+Alt+V**（或右键「预览 Markdown」），支持 Mermaid 图表、LaTeX 公式、代码高亮；**Ctrl+F** 在预览中搜索；**Alt+单击**预览内容跳转到 `.md` 源码对应行；点击「导出 HTML」生成自包含文件，离线可查看。
...
| 快捷键 | 功能 | 说明 |
|--------|------|------|
| `Ctrl+Alt+V` | 打开 Markdown 预览 | 仅 Markdown 文件；不占用 VS Code 原生 `Ctrl+Shift+V` |
